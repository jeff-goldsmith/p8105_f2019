...
This is the second module in the [Data Wrangling II](topic_data_wrangling_ii.html) topic; the relevant slack channel is [here](https://p8105fall2019.slack.com/messages/CN8998SH5).
...
It would get really tedious to try to scrape all (~1400) reviews by copying code over and over; before long we'll learn about writing R functions to reuse code and make this much easier!
...
We've seen factors a few times already, especially in [Viz Pt 2](visualization_pt2.html). In that case, we were interested in reordering a factor variable in order to produce clearer plots -- `ggplot` uses factor levels to determine the order in which categories appear. Let's revisit those examples now. 
...
We also saw factors in our exploratory analysis of the [Airbnb](dataset_airbnb.html) data, when looking at the distribution of prices in various neighborhoods. Ordering these according to the median price made for clearer plots than ordering neighborhoods alphabetically. 
