...
This is the second module in the [Visualization and EDA](topic_visualization_and_eda.html) topic; the relevant slack channel is [here](https://p8105fall2019.slack.com/messages/CN01ALHR7). Note that the slides and Other Materials / Extra Reading is the same as in [Visualization Pt 1](visualization_pt1.html).
...
I'll create a new .Rmd for this example, and include it in the Git repo / directory for the [Visualization and EDA](topic_visualization_and_eda.html) topic. I'm also going to take advantage of features in additional packages; most of these were installed previously, but it might be necessary to install [`patchwork`](https://github.com/thomasp85/patchwork) using the code below:

```{r, eval = FALSE}
devtools::install_github("thomasp85/patchwork")
```

Next I'll load the relevant packages for this example. 
...
Analogously to `scale_x_*` and `scale_y_*`, there are `scale`s corresponding to many other aestheics. Some of the most common are used to control the `color` aesthetic. For example, arguments to `scale_color_hue()` control the color scale and the name in the plot legend. 
...
There are ways to set color preferences globally as well, although they're a bit more complex and we probably won't cover them. 
...
The package is already very helpful but is under active development -- some features may change or be added over time -- so check back periodically to stay up-to-date. This does require some amount of work for each panel, and you should keep in mind that this is intended to help combine panels into a plot that couldn't be created better using data tidying steps. 
