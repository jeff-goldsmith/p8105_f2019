...
Download and install the latest versions of [R](https://cran.r-project.org) (3.6.1) and [RStudio](https://www.rstudio.com/products/rstudio/download/) (1.2.1335). 

Open RStudio and install several packages (and their dependencies) using the code below:

```{r eval = FALSE}
install.packages(c("tidyverse", "knitr", "rmarkdown", "janitor", "broom", "here", "readxl", "haven", "rnoaa", "ggridges", "ggthemes", "leaflet", "viridis", "skimr", "rvest", "httr", "flexdashboard", "devtools", "usethis", "modelr", "mgcv", "tidytext", "shiny"))
...
Upload a screenshot of RStudio's terminal pane that shows a successful execution of `which git` or `git --version`. Here's an example "solution":
