...
This is the third module in the [Data Wrangling I](topic_data_wrangling_i.html) topic; the relevant slack channel is [here](https://p8105-fall2018.slack.com/messages/CCAEZLU49).
...
This isn't really the optimal way to isolate "visit" because I now need an additional `select` step to remove the unnecessary character variables `remove_1` and `remove_2`. However, it does illustrate the `separate` function, and we haven't yet learned about `str_replace` and other functions from the `stringr` package that are better suited to this problem, so ...
...
**_Learning Assessment:_** The datasets in [this zip file](data/survey_results.zip) contain de-identified responses to surveys included in Homeworks 0 and 1. Both contain a unique student identifier; the first has reponses to a question about operating systems, and the second has responses to questions about degree program and git experience. Write a code chunk that imports and cleans both datasets, and then joins them. 
...
`gather` and `spread` aren't really intuitive names for what the functions do. As a result, `tidyr` [is being updated](https://tidyr.tidyverse.org/dev/articles/pivot.html) to include `pivot_longer` and `pivot_wider` -- names that are at least more in keeping with how this kind of tidying is described elsewhere. `gather` and `spread` will still exist, but they're going to be less common over time. 
