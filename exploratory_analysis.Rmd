...
This is the second module in the [Visualization and EDA](topic_visualization_and_eda.html) topic; the relevant slack channel is [here](https://p8105-fall2018.slack.com/messages/CCCBZPUVC).
...
This is a "tidy" table, and it's also a data frame. You can re-organize into a more standard 2x2 table using tools we'll see later, but instead I suggest using `janitor::tabyl`:
...
(Since we're on the subject, I think 2x2 tables are kind of silly. When are you ever going to actually analyze data in that format?? In grad school I thought I'd be computing odds ratios by hand everyday ($OR = AD / BC$!!), but really I do that about as often as I write in cursive -- which is also never. Just do a logistic regression adjusting for confounders -- because there are _always confounders_. And is a 2x2 table really that much better than the "tidy" version? There are **_4 numbers_**. )
...
**_Learning Assessment:_** In the FAS data, there are several outcomes of interest; for now, focus on post-natal day on which a pup is able to pivot. Two predictors of interest are the dose level and the day of treatment. Produce a reader-friendly table 
...
The code that I produced working examples in lecture is [here](https://github.com/P8105/viz_and_edap).