...
This is the first module in the [Building Blocks](topic_building_blocks.html) topic; the relevant slack channel is [here](https://p8105fall2019.slack.com/messages/CMPF9U6LA).
...
class(pull(example_df, vec_numeric))
class(pull(example_df, vec_factor))
```
...
Create a directory and save the previous two scripts into that directory. Script names, like variable names, should be descriptive (e.g. `20190905_data_classes.R` and `20190905_simple_plots.R`). The directory name should be descriptive as well, and it should be in a reasonable place on your computer (e.g. `~/Documents/First_Project/`). Create an R Project using File > New Project > Existing Directory and specifying the directory you just created.
